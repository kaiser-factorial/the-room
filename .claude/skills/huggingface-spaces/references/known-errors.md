...
### `useradd: UID 1000 is not unique` (Docker Spaces)

HF Docker Spaces docs suggest `RUN useradd -m -u 1000 user`, but many base
images already ship a uid-1000 user — `node:*` images have `node`,
`python:*-slim` sometimes has one too. Reuse the existing user instead of
creating one:

```dockerfile
# node images: user 'node' is already uid 1000
USER node
ENV HOME=/home/node
...
(Or `useradd -m -u 1001 user` if you truly need a fresh one — the platform
only cares that the app can write where it needs to.)

### Docker Space builds fine, then shows RUNTIME_ERROR with a healthy-looking log — daemon with no HTTP listener

Docker Spaces health-check the container by connecting to `app_port`
(default 7860). A background worker / poller / queue daemon that serves no
HTTP will be marked broken even though the process is running. Add a
minimal status endpoint and set `app_port` in the README frontmatter:

```js
// node example — any stack works, it just has to answer on $PORT
require('http').createServer((_, res) => res.end('ok'))
  .listen(process.env.PORT || 7860);
```

Also remember a crash-looping daemon (e.g. exiting on missing env/secret)
shows as RUNTIME_ERROR with the real reason only in
`hf spaces logs <id> --tail 50` — read the runtime log, not just the stage.
