...
const S = (over: Partial<SearchConfig> = {}): SearchConfig => ({ enabled: true, gated: false, notice: true, maxResults: 5, ...over });
...
test('condition presets: search-tool and gated resolve onto the base config', async () => {
  ...
  assert.deepEqual(tool.search, { enabled: true, gated: false, notice: true, maxResults: 5 });
